...
from models.encoders.dinov2.layers import MemEffCrossAttention, SelfAttention
...
class ImageModel(nn.Module):
    ...
    def __init__(self, encoder, decoder,
        ...
        # self.prompt_cross_attn = SelfAttention(hidden_dim)
    ...
    def forward(self, samples: NestedTensor, targets, sam2_image_model=None, sat_use_gt = False, detach_j3ds = False):
        ...
        # sam2_feats, all_sparse_embeddings, all_dense_embeddings, logits = sam2_image_model(
        #     samples, 
        ...
        # )
        # del sam2_feats, all_sparse_embeddings, all_dense_embeddings 
        ...
        # prompt 추가 - copy from current one
        # Add prompt part 
        # DL
        # Train
        ...
        # # mask_batch = torch.cat([mask_to_box(torch.from_numpy(logit).to(dtype=torch.bool, device=device)) for logit in logits])
        # # mask_batch = []
        # # for logit in logits:
        # #     tensor = torch.from_numpy(logit).to(dtype=torch.bool, device=device)
        # #     if tensor.dim() == 3:
        # #         tensor = tensor.unsqueeze(0)
        # #     mask_batch.append(mask_to_box(tensor))
        # # mask_batch = torch.cat(mask_batch)
        # # # mask_boxes = box_xyxy_to_cxcywh(mask_batch)[:, 0, :] / self.input_size
        # # mask_boxes = mask_batch[:, 0, :] / self.input_size

        # # tgt_embed = (self.pr_tgt_embed.weight).unsqueeze(0)
        # # tgt_embed = tgt_embed.repeat(1, pr_tgt.shape[1], 1)
        # pr_tgt, pr_points, pr_attn_mask, prompt_dn_meta =\
        #                     prepare_prompt_query_box(targets, self.dn_cfg, num_queries=self.num_queries, 
        #                     hidden_dim = self.hidden_dim, prompt_encoder=None, dn_prompt_enc = self.dn_prompt_enc,
        #                     # hidden_dim = self.hidden_dim, prompt_encoder=self.prompt_encoder, dn_prompt_enc = self.dn_prompt_enc,
        #                     # # hidden_dim = self.hidden_dim, prompt_encoder=self._prompt_encoder, dn_prompt_enc = self.dn_prompt_enc,
        #                     # sparse_embeddings=all_sparse_embeddings)
        #                     # hidden_dim = self.hidden_dim, mask_encoder=self.mask_encoder,
        #                     # mask_boxes=mask_boxes, prompt_emb=tgt_embed)
        #                     mask_boxes=None) #, prompt_emb=tgt_embed)
        # # import pdb; pdb.set_trace()
        # # self.mask_encoder = self.mask_encoder.to(torch.float32)
        # # self.mask_encoder(torch.from_numpy(logits[0]))
        # # import pdb; pdb.set_trace()
        # embedweight = (self.refpoint_embed.weight).unsqueeze(0).repeat(bs,1,1)
        # tgt = (self.tgt_embed.weight).unsqueeze(0).repeat(bs,1,1)

        
        # # tgt_embed = torch.cat([pr_tgt, tgt_embed], dim=-1) 
        # # pr_tgt = self.pr_embed_enc(tgt_embed)
        ...
        # mask_batch = [mask_to_box(torch.from_numpy(logit).to(dtype=torch.bool, device=device))  if logit.ndim == 4 
        #                 else mask_to_box(torch.from_numpy(logit)[None].to(dtype=torch.bool, device=device)) for logit in logits]
        # person_cnt = [mask.shape[0] for mask in mask_batch]
        # max_persons = max(person_cnt)

        # # masks_tgt = torch.zeros((max_persons, self.hidden_dim), device=device)
        # mask_boxes = torch.zeros((bs, max_persons, 4), device=device)

        # for idx, mask in enumerate(mask_batch):
        #     if len(mask.shape) == 3:
        #         boxes = box_xyxy_to_cxcywh(mask)[:, 0, :] / self.input_size 
        #     else:
        #         boxes = box_xyxy_to_cxcywh(mask) / self.input_size 
        #     mask_boxes[idx] = boxes.repeat(max_persons // len(boxes) + 1, 1)[:max_persons]

        # eps = 1e-4
        # pr_points = inverse_sigmoid(mask_boxes.clamp(min=eps, max=1 - eps))

        # # B, N, D = embedweight.shape
        # # K = pr_points.shape[1]
        # # C = tgt.shape[2]

        # # diff = embedweight.unsqueeze(2) - pr_points.unsqueeze(1)  # [B, N, K, D]
        # # dist = (diff ** 2).sum(-1)                                # [B, N, K]

        # # r = 4
        # # tau = 0.1
        # # topk_vals, topk_idx = torch.topk(-dist, k=min(r, N), dim=1) 
        # # weights = F.softmax(topk_vals / tau, dim=1)  # [B, r, K]

        # # flat_idx = topk_idx.reshape(B, r*K)  # [B, r*K]

        # # # gathered_ref = embedweight.gather(
        # # #     1, flat_idx.unsqueeze(-1).expand(-1, -1, D)   # [B, r*K, D]
        # # # ).reshape(B, r, K, D)                              # [B, r, K, D]

        # # gathered_tgt = tgt.gather(
        # #     1, flat_idx.unsqueeze(-1).expand(-1, -1, tgt.size(2))  # [B, r*K, C]
        # # ).reshape(B, r, K, tgt.size(2))

        # # # new_ref = (weights.unsqueeze(-1) * gathered_ref).sum(1)  # [B, K, D]
        # # pr_tgt = (weights.unsqueeze(-1) * gathered_tgt).sum(1)  # [B, K, C]
        # # # prompt_dn_meta['pad_size'] = pr_tgt.shape[1]

        # # self.bbox_weight_mlp = MLP(4, 256, 256, 3)
        # # self.modulated_weight_mlp = MLP(256, 256, 50, 3)

        # # weights = self.modulated_weight_mlp(self.bbox_weight_mlp(mask_boxes))
        # # # import pdb; pdb.set_trace()
        # # weights = torch.softmax(weights, dim=-1)   
        # # tgt_for_mlp = self.tgt_embed.weight.detach().unsqueeze(0).repeat(bs, 1, 1)
        # # pr_tgt = torch.matmul(weights, tgt_for_mlp)   

        # # B = mask_boxes.shape[0]
        # # m = mask_boxes.shape[1]
        # # n, D = self.tgt_embed.weight.shape

        # # r = n // m                         # 몫만 사용
        # # remain = n % m                     # 나머지

        # # if remain > 0:
        # #     tgt_for_mlp = self.tgt_embed.weight[:-remain]
        # # else:
        # #     tgt_for_mlp = self.tgt_embed.weight

        # # tgt_for_mlp = tgt_for_mlp.unsqueeze(0).expand(B, -1, D)
        # # tgt_for_mlp = tgt_for_mlp.view(B, m, r, D)

        # # weights = self.modulated_weight_mlp(self.bbox_weight_mlp(mask_boxes))  # [B, m, r]

        # # weights = torch.softmax(weights, dim=-1)            # [B, m, r]
        # # import pdb; pdb.set_trace()
        # # pr_tgt = (weights.unsqueeze(-1) * tgt_for_mlp).sum(dim=2) 
        # import pdb; pdb.set_trace()

        # prompt_pad_size = pr_tgt.shape[1]

        # tgt_size = prompt_pad_size + self.num_queries
        # pr_attn_mask = torch.zeros((tgt_size, tgt_size), dtype=bool, device=device)

        # # match queries cannot see DN (reconstruct) tokens
        # pr_attn_mask[prompt_pad_size:, :prompt_pad_size] = True

        # # reconstruct groups cannot see each other (각 그룹 내부만 False=허용)
        # for g in range(self.dn_cfg['dn_number']):
        #     g_start = prompt_pad_size * g
        #     g_end   = g_start + prompt_pad_size
        #     if g_start > 0:
        #         pr_attn_mask[g_start:g_end, :g_start] = True
        #     if g_end < prompt_pad_size:
        #         pr_attn_mask[g_start:g_end, g_end:prompt_pad_size] = True
